...
- [x] **WS-7.5** Point offline downloads at `LocalStyleServer` instead of GitHub (Commit: 7d4938f)
  - **Repos:** wingspan
...
- [x] **WS-8.2** Add the snapshot list and detail screens (Commit: 0b5ca81)
  - **Repos:** wingspan
...
- [x] **WS-8.3** Share a snapshot through the Android share sheet (Commit: 75c5967)
  - **Repos:** wingspan
...
- [x] **WS-8.4** Show a saved snapshot on the map (Commit: bd2019b)
  - **Repos:** wingspan
...
- [x] **WS-9.2** Persist the current report and add the report view model (Commit: 27d38fc)
  - **Repos:** wingspan
...
- [x] **WS-9.3** Draw report shots on the map and record them by tapping (Commit: 1ceaef3)
  ...
  - **Instructions:**
    ...
    5. **Mode interactions.** The map screen now has four independent full-screen modes that can each be active - manual position (`manualMode`), the zone editor (`EditorViewModel.EditorMode`), snapshot viewing (`viewingSnapshot`, WS-8.4), and Report mode (`reportMode`) - and every one of them must explicitly define its relationship to every other one rather than assuming they are independent or mutually exclusive by default; an unspecified interaction between manual position and snapshot viewing already caused a real device-tested bug (WS-8.4 originally reset manual mode back to GPS by silently recreating `MapViewModel`, diagnosed and fixed only after the fact), and this must not recur for Report mode. Specifically: (a) turning Report mode on or off must never change `manualMode`, the manually-placed position, or force a GPS reacquisition - `addPosition` always uses whatever `fanState.origin`/`shooter` currently holds, GPS or manual, matching manual mode's own stated purpose of planning away from the range; (b) the "Report mode" `MapMenu` item's action in `MapScreen.kt` must no-op with a toast such as "Finish or cancel the current edit first" (rather than silently entering Report mode) whenever `editorMode` is not `EditorViewModel.EditorMode.Idle`, making the existing "hide Add zone while report mode is active" guard bidirectional; (c) turning Report mode on must also call `closeSnapshotView()` when `viewingSnapshot` is non-null, and the existing snapshot-viewing navigation path from WS-8.4 (the `onShowOnMap` callback and the `LaunchedEffect(snapshotId)` it drives) must also call `reportViewModel.setReportMode(false)`, so the amber snapshot overlay and `ReportControls` never compete for the map screen at the same time.
  - **Verify:** `cd /home/nathan/wingspan && ./gradlew assembleDebug testDebugUnitTest`
...
- [x] **WS-9.6** Render the basemap for the PDF and export it through the file picker (Commit: bb1437a)
  - **Repos:** wingspan
...
- Offline downloads reference an `http://127.0.0.1:<port>` style URL served by the in-process `LocalStyleServer` (`Basemap.remoteStyleUrl(port)`, added WS-7.4/7.5), not GitHub - this replaced an earlier GitHub-raw-content approach that broke whenever `origin/main` fell behind what was actually on the device. The live map separately loads the identical style JSON from local assets via `Style.Builder().fromJson(...)`, unchanged. Keep the two in sync — they read the same underlying asset files through two different code paths.
...
- Deferred to later phases: ARCore AR overlay (product Phase 2), KML import/export, vertex snapping/undo history for edits, persisting the manual position across restarts, altitude/temperature corrections to the ballistics model, buckshot sizes.
